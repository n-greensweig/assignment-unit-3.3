...
for (let i = 0; i < supplyChanges.length; i++) {
  let supplyChange = supplyChanges[i];
  if (supplyChange > 0) {
    positives.push(supplyChange);
  } else if (supplyChange < 0) {
    negatives.push(supplyChange);
  } else {
    zeroes.push(supplyChange);
  }
}
...
let parts = 572;
let boxesFilled = 0;
let maxBoxes = Math.floor(parts / 7);

while (boxesFilled <= maxBoxes) {
  boxesFilled += 1/7;
  parts--;
  
}
boxesFilled = Math.round(boxesFilled);
console.log(parts);
